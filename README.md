...
- Storage Access Framework 多目录持久只读授权；没有完整媒体权限时也可只使用授权目录
...
- 系统媒体控件、锁屏、耳机、蓝牙、音频焦点和耳机拔出处理
- 播放、暂停、Seek、上一首、下一首、随机与循环
- 可视化队列：跳转、上下调整、移除与确认清空
- 持久化队列、当前项、位置、随机和循环状态
- 服务重建后恢复播放上下文并保持暂停
- 真实专辑封面、音频内嵌图片回退、尺寸采样和受限 LRU 缓存

### 完成度、统计与隐私

- PlaybackService 按真实播放会话采样收听时长，后台播放和桌面小组件控制同样进入统计
- 明显 Seek 跳转不计入有效收听；完成判定同时要求自然到达曲尾和足够收听时长
- 每首歌曲保存播放次数、最后播放、完成次数、最后完成、累计收听时长和可恢复中途位置
- 设置页提供总播放、总完成、总收听时长，以及收听时长最高的歌曲、艺术家和专辑
- 可通过系统 CreateDocument 导出带 schema 版本的 UTF-8 JSON 或 CSV
- 导出保留当前不可用媒体 ID，避免 SD 卡、USB 或 SAF 暂离线时丢失统计
- 可分别清除最近播放和播放统计；收藏、歌单、歌词、主题与音乐文件保持不变
- 兼容已有 `playback_stats_v1` 和 `playback_stats_v2` 数据

### 桌面播放小组件

- Jetpack Glance 桌面播放小组件
- 显示当前曲目和艺术家
- 上一首、播放/暂停、下一首直接连接现有 MediaSession
...
- M3U/M3U8 导入、UTF-8 M3U8 导出、导入预览和歧义项手工映射
- 跨设备 MediaStore ID 与 `content://` URI 碰撞保护
- 可用歌曲导出为便携相对路径，并保留同设备精确 URI 注释
- 旧 MediaStore Playlist 支持一次性只读迁移，不回写或删除系统歌单
...
- 独立 `:benchmark` 模块，使用 release-like、不可调试的 benchmark 变体
- Baseline Profile 与 Startup Profile 生成并合入应用
...
modern-app/                       活动 Compose 应用
  src/main/kotlin/.../data/       MediaStore、SAF、统计、歌单、M3U 与性能夹具
  src/main/kotlin/.../lyrics/     LRC 解析、存储与歌词状态
  src/main/kotlin/.../playback/   Media3、队列、恢复和播放进度采样
  src/main/kotlin/.../widget/     Glance 小组件与 MediaSession 动作
  src/main/kotlin/.../ui/         Compose 页面、统计和编辑器
  src/test/                       JVM 单元测试
...
生成 Profile 并运行性能/恢复套件：

```bash
./gradlew --no-daemon :app:generateBaselineProfile
./gradlew --no-daemon :benchmark:pixel2Api35BenchmarkBenchmarkAndroidTest
```
...
- M3U/M3U8、歌词和统计导出：系统文件选择器
- Android 13+ 播放通知：`POST_NOTIFICATIONS`
- 后台播放：`FOREGROUND_SERVICE_MEDIA_PLAYBACK`

应用不申请共享存储写权限，不启用明文网络，也不使用 legacy external storage。收藏、历史、统计、歌词、歌单和小组件快照均保存在设备本地。
