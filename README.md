...
ACG Player X 2.0 是一次面向现代 Android 的重写。当前活动应用已经迁移到 **Jetpack Compose + Material 3 + Media3**；旧版 Java/XML/Fragment 源码仍保留为迁移参考，但不参与默认构建。

当前开发版本：**2.0.0-alpha09**。
...
- 未授予完整媒体权限时，仍可只浏览和播放用户授权目录
- 递归读取 MediaStore 未索引音频，并与系统媒体库合并、去重
- 按歌曲、收藏、最近播放、最近添加、最常播放、未播放、专辑、艺术家和文件夹浏览
- 跨标题、艺术家、专辑、文件夹名称和可读目录路径搜索
- 精确进入专辑、艺术家或文件夹，并继续在集合内搜索
...
- 搜索歌单内歌曲
- 单曲添加、添加当前搜索结果、移出、上下调整和清空歌单
- 批量选择当前歌单歌曲，支持按搜索结果全选或取消当前结果
- 批量将所选歌曲移到顶部、移到底部或移出歌单；移动时保持所选歌曲原有相对顺序
- 单曲移出、上下排序、批量排序、批量移出、清空和清理失效项均支持 Snackbar 单步撤销
- 按歌单顺序播放或随机播放
- 统计可用歌曲、总时长和当前不可用项目
- SAF 目录暂时离线或权限失效时，仅隐藏当前不可用歌曲，不破坏歌单结构
- 可显式清理当前不可用项目；不会删除设备上的音频文件
- 通过系统文件选择器导入 M3U/M3U8，并导出标准 UTF-8 M3U8
- 导入时按 ACG 媒体 ID、内容 URI、文件名/目录、标题/艺术家/时长逐级匹配；多候选时不会猜测
- 导出时写入标准 `#EXTINF`，并附带可忽略的 ACG 元数据，支持跨 MediaStore、SAF 和设备恢复
...
- UTF-8/BOM 与 GB18030 歌词读取，导入后复制到应用内部存储
...
modern-app/                       当前活动的 Compose 应用
  ...
  src/main/kotlin/.../ui/         Compose 页面、批量歌单编辑、封面和通用组件
app/                              旧版应用源码，仅供迁移参考
...
GitHub Actions 会在推送和 Pull Request 时执行相同门禁。单元测试覆盖搜索、Android 新旧路径、MediaStore 时间回退、SAF 音频识别、稳定文档 ID、跨来源去重、收藏、播放统计、智能列表、歌单编解码、批量移除和顺序变换、M3U 解析与重匹配，以及 LRC 时间轴。
...
系统媒体权限和 SAF 目录权限相互独立。用户可以不授予完整媒体权限，只授权特定目录；应用会保存目录 URI，但不会上传路径或音频内容。移除目录时会释放对应持久权限。

收藏、历史、播放统计和自定义歌单只保存在设备本地。歌单保存名称、时间戳和媒体 ID 顺序，不复制音乐文件。M3U 导入只读取用户选择的文件，导出只写入用户选择的目标。应用不请求共享存储写权限、不启用明文网络，也不使用 `requestLegacyExternalStorage`。

## 歌单编辑与撤销语义

批量选择器只展示当前可用歌曲，但操作始终基于歌单保存顺序执行。批量置顶或置底不会按勾选先后重新排序，而是保持这些歌曲在原歌单中的相对顺序。

破坏性歌单操作会保存一次内存快照，并通过 Snackbar 提供单步撤销：
...
撤销快照仅存在于当前应用进程中；完成新的歌单修改、关闭对应歌单或 Snackbar 超时后，旧快照会被释放。撤销只恢复歌单媒体 ID 顺序，不会改动设备上的音频文件。
...
M3U8 导出以 UTF-8 写入标准 `#EXTM3U`、`#PLAYLIST` 与 `#EXTINF`。额外的 `#ACGPLAYER-*` 注释可被其他播放器安全忽略，同时保存媒体 ID、文件名和目录提示。当前不可用项目使用 `acg-player://media/<id>` 占位，因此重新导入本应用时仍可保留顺序。

导入支持 UTF-8、UTF-8 BOM，并在解码失败时回退 GB18030。匹配顺序为：经过可移植提示校验的媒体 ID、经过校验的精确内容 URI、文件名与目录、标题/艺术家/时长。存在多个候选时条目会被标记为歧义并跳过。单文件限制为 4 MB，最多解析 20,000 条。
...
本地播放器核心闭环、MediaStore 与 SAF 双来源音乐库、自定义歌单、批量歌单编辑与撤销、M3U/M3U8 互操作、真实及内嵌封面、媒体库自动刷新、播放状态恢复、同步 LRC 歌词、可视化队列、收藏、最近播放和基础智能列表已经迁入 Compose 模块。

仍待迁移的重点包括长按拖拽排序、M3U 导入预览与手工映射、历史 MediaStore Playlist 只读导入、规则智能列表、歌词编辑、联网音乐 Provider、音频标签编辑、Glance 小组件、Live2D、购买流程和发布级性能/仪器化测试。
