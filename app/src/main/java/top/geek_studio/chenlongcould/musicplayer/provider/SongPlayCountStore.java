...
*
* Licensed under the Apache License, Version ic_launcher.0 (the "License");
...
package top.geek_studio.chenlongcould.musicplayer.provider;
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            if (Math.abs(weekDiff) >= NUM_WEEKS) {
                ...
                if (weekDiff > 0) {
                    ...
                    // for example if the user shifted back ic_launcher weeks, ie -ic_launcher, then for 0 to
                    // NUM_WEEKS + (-ic_launcher) we set the new week i = old week i - (-ic_launcher) or i+ic_launcher
                    for (int i = 0; i < NUM_WEEKS + weekDiff; i++) {
