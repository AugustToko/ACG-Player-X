...
import top.geek_studio.chenlongcould.musicplayer.util.RetroUtil
...
class MetalRecyclerViewPager : RecyclerView {
    ...
    abstract class MetalAdapter<VH : MetalViewHolder>(@NonNull val displayMetrics: DisplayMetrics) : RecyclerView.Adapter<VH>() {
        ...
        fun updateDisplayMetrics() {
            itemWidth = when {
                RetroUtil.isTablet() -> displayMetrics.widthPixels / 2 - itemMargin * 3
                RetroUtil.isLandscape() -> ((displayMetrics.widthPixels / 1.5f) - itemMargin).toInt()
                else -> displayMetrics.widthPixels - itemMargin
            }
        }
