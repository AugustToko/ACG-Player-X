...
1. **JVM 单元测试**：路径、合并去重、排序、歌单、M3U、LRC、播放状态编解码、Glance 文本状态和 benchmark 夹具选择。
2. **构建门禁**：Android Lint、JVM 测试、Debug APK、AndroidTest APK 与 benchmark APK。
3. **应用设备测试**：API 35 AOSP ATD 上运行 Compose、MediaSession、Glance 状态、ContentResolver 和应用文件系统测试。
...
| `PlaybackServiceInstrumentedTest` | MediaController 连接 MediaSession、初始暂停、播放命令和当前元数据写入小组件快照 |
| `PlaybackWidgetStateStoreInstrumentedTest` | 小组件空状态、跨 Store 实例持久化、Receiver 导出状态和 appwidget provider metadata |
...
小组件快照独立于完整播放恢复 JSON，仅保存渲染所需字段。Timeline、媒体切换、元数据和 `isPlaying` 变化采用 180ms 去抖，避免快速事件风暴导致连续 RemoteViews 更新。
...
- Glance 颜色使用公开日夜 ColorProvider API，不使用 Lint 禁止的资源型受限接口。
- Glance 启动 Activity 使用显式 Intent，兼容 1.2 API。
